...
def visualize_path_with_network(nodes: Dict[str, Coord], edges: List[EdgeItem], path: List[Coord], street_names: List[str], figsize: Tuple[int, int] = (15, 15)):
    ...
    ax.autoscale()
    plt.axis('equal')
    plt.title('Кратчайший маршрут по Софии')
    plt.xlabel('Долгота')
    plt.ylabel('Широта')
    plt.grid(False)
    plt.tight_layout()
...
if __name__ == "__main__":
    nodes, edges = read_graphml("sofia_road_network.graphml")
    graph = build_graph(edges)

    routes = [
        ("Самоковско шосе", "Гюешево"),
        ("Проф. Александър Станишев", "Леонардо да Винчи"),
        ("Цар Иван Асен II", "бул. Сливница")
    ]
